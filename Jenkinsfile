pipeline {
  ...
  environment {
        ...
        FTEST_DIR = "eea/progress.workflow/ftests"
    }
          ...
          "Plone5": {
            node(label: 'docker') {
              script {
                try {
                  checkout scm
                  sh '''docker run -d -e ADDONS=$GIT_NAME -e DEVELOP=src/$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" --name=$BUILD_TAG-ft-plone5 eeacms/plone-test:5'''
                  sh '''timeout 600  wget --retry-connrefused --tries=60 --waitretry=10 -q http://$(docker inspect --format {{.NetworkSettings.IPAddress}} $BUILD_TAG-ft-plone5):8080/'''
                  sh '''casperjs test $FTEST_DIR/plone5/*.js --url=$(docker inspect --format {{.NetworkSettings.IPAddress}} $BUILD_TAG-ft-plone5):8080 --xunit=ftestsreport.xml'''
                } catch (err) {
                sh '''docker logs --tail=100 $BUILD_TAG-ft-plone5'''
                throw err
                } finally {
                  sh '''docker stop $BUILD_TAG-ft-plone5'''
                  sh '''docker rm -v $BUILD_TAG-ft-plone5'''
                }
              }
              archiveArtifacts '*.png'
              junit 'ftestsreport.xml'
